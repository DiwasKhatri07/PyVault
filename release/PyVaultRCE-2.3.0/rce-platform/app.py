...
def admin():
    ...
    rows = db.execute(
        ...
        "expires_at, CAST(COALESCE(max_executions, 0) AS INTEGER) AS max_executions, label "
...
def _do_save(code: str, expires_in_hours=None, max_executions=0):
    ...
    if len(code.encode("utf-8")) > MAX_CODE_BYTES:
        return None, None, None, f"Code exceeds the {MAX_CODE_BYTES:,}-byte upload limit."
    ...
    db.commit()
    return session_id, owner_token, expires_at, None
...
def api_save():
    ...
    sid, owner_token, expires_at, err = _do_save(code, expires_in_hours, max_executions)
    if err:
...
def api_upload():
    ...
    sid, owner_token, expires_at, err = _do_save(code)
    if err:
        return jsonify({"error": err}), 400
