...
function showResult(sid, ownerToken, lineCount, expiresAt, shareUrl, libraries) {
...
CodeManager.run("${sid}")`;
  ...
  if (ownerToken) {
    ...
    document.getElementById('myProfileSid').value  = sid;
  } else {
    ownerTokenBox.style.display = 'none';
  }
  if (shareUrl) {
    shareLinkText.textContent = shareUrl;
    shareLinkText.href = shareUrl;
...
saveBtn.addEventListener('click', async () => {
  ...
  const libraries  = document.getElementById('librariesInput')?.value.trim() || '';
  const username   = usernameInput?.value.trim() || '';
  const tags       = tagsInput?.value.trim() || '';
  const description = descriptionInput?.value.trim() || '';

  const body = { code, label, libraries, username, tags, description };
  ...
  try {
    ...
    showResult(data.session_id, data.owner_token, lineCount, data.expires_at || null, data.share_url, data.libraries);
  } catch (e) {
...
makeCopyHandler(copyShareBtn, () => shareLinkText.href || shareLinkText.textContent);
...
// ── Owner: Update public profile metadata ─────────────────────────────────────

myProfileBtn?.addEventListener('click', async () => {
  const sid = document.getElementById('myProfileSid').value.trim();
  const token = document.getElementById('myProfileToken').value.trim();
  const st = document.getElementById('profileStatus');
  if (!sid || sid.length !== 21) { st.textContent = '✕ Enter a valid 21-char Session ID.'; st.style.color = 'var(--red)'; return; }
  if (!token) { st.textContent = '✕ Owner token is required.'; st.style.color = 'var(--red)'; return; }
  const body = {
    username: document.getElementById('myProfileUsername').value,
    label: document.getElementById('myProfileLabel').value,
    tags: document.getElementById('myProfileTags').value,
    description: document.getElementById('myProfileDescription').value,
  };
  try {
    ...
    const data = await res.json();
    if (res.ok) { st.textContent = '✓ Public profile updated.'; st.style.color = 'var(--green)'; }
    else { st.textContent = '✕ ' + data.error; st.style.color = 'var(--red)'; }
  } catch (e) {
    st.textContent = 'Network error: ' + e.message; st.style.color = 'var(--red)';
  } finally {
