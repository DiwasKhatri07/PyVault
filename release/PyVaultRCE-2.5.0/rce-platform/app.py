...
MAX_TAGS_LENGTH = 500
MAX_COMMENT_LENGTH = 1_200
AUTO_VERIFY_VIEWS = 200
...
def init_db():
    with app.app_context():
        ...
        db.execute(
            "UPDATE sessions SET username = 'anonymous' "
            "WHERE username IS NULL OR TRIM(username) = ''"
        )
        db.execute(
            "INSERT OR IGNORE INTO profiles (username) "
            "SELECT DISTINCT username FROM sessions WHERE username IS NOT NULL"
        )
        db.commit()
...
def _normalize_username(value) -> str:
    """Keep public usernames predictable and safe to use in profile URLs."""
    username = str(value or "").strip().lstrip("@").lower()
    if not re.fullmatch(r"[a-z][a-z0-9_]{2,23}", username):
        return "anonymous"
    if username in {"admin", "api", "pyv", "s", "u", "static", "favicon"}:
        return "anonymous"
    return username
...
def _normalize_tags(value) -> str:
    ...
    for item in raw_values:
        ...
        if not tag or not re.fullmatch(r"[a-z0-9][a-z0-9_-]{0,31}", tag):
            continue
        if tag not in cleaned:
            cleaned.append(tag)
    return ", ".join(cleaned[:20])[:MAX_TAGS_LENGTH]
...
def _profile_verified(username: str, views=0) -> bool:
    db = get_db()
    row = db.execute(
        "SELECT verified FROM profiles WHERE username = ?", (username,)
    ).fetchone()
    manually_verified = bool(row and row["verified"])
    try:
        automatically_verified = int(views or 0) >= AUTO_VERIFY_VIEWS
    except (TypeError, ValueError):
        automatically_verified = False
    return manually_verified or automatically_verified


def _visitor_key() -> str:
    raw = f"{request.remote_addr or 'unknown'}|{request.headers.get('User-Agent', '')[:240]}"
    return hashlib.sha256(raw.encode("utf-8")).hexdigest()


def _record_view(session_id: str) -> bool:
    """Count at most one view per visitor fingerprint per UTC day."""
    db = get_db()
    today = datetime.utcnow().date().isoformat()
    try:
        result = db.execute(
            "INSERT OR IGNORE INTO view_events "
            "(session_id, visitor_key, viewed_on, created_at) VALUES (?, ?, ?, ?)",
            (session_id, _visitor_key(), today, datetime.utcnow().isoformat()),
        )
        if result.rowcount:
            db.execute(
                "UPDATE sessions SET views = COALESCE(views, 0) + 1 WHERE session_id = ?",
                (session_id,),
            )
            db.commit()
            return True
...
def _delete_session_data(session_id: str):
    db = get_db()
    db.execute("DELETE FROM comments WHERE session_id = ?", (session_id,))
    db.execute("DELETE FROM view_events WHERE session_id = ?", (session_id,))
...
def public_share(share_slug):
    if not re.fullmatch(r"[0-9a-f]{8}", share_slug):
        abort(404)
    db = get_db()
    # View counting is deduplicated by a daily visitor fingerprint, not by
    # refreshes, so one person cannot inflate the public verification signal.
    _record_view_for_slug = db.execute(
        "SELECT session_id FROM sessions WHERE share_slug = ?", (share_slug,)
    ).fetchone()
    if not _record_view_for_slug:
        abort(404)
    _record_view(_record_view_for_slug["session_id"])
    row = db.execute(
        "SELECT session_id, created_at, expires_at, execution_count, max_executions, "
        "label, libraries, share_slug, username, description, tags, views "
        "FROM sessions WHERE share_slug = ?",
        (share_slug,),
    ).fetchone()
    if not row:
        abort(404)
    ...
    data["tags_list"] = [x for x in (data.get("tags") or "").split(", ") if x]
    data["profile_verified"] = _profile_verified(data.get("username"), data.get("views"))
    data["auto_verified"] = int(data.get("views") or 0) >= AUTO_VERIFY_VIEWS
    data["comments"] = [
        dict(comment)
...
def public_profile(username):
    ...
    db = get_db()
    profile = db.execute(
        "SELECT username, verified, created_at FROM profiles WHERE username = ?",
        (normalized,),
    ).fetchone()
    posts = db.execute(
        "SELECT session_id, share_slug, label, description, tags, views, created_at "
        "FROM sessions WHERE username = ? ORDER BY created_at DESC LIMIT 100",
        (normalized,),
    ).fetchall()
    if not profile and not posts:
        abort(404)
    profile_data = dict(profile) if profile else {
        "username": normalized, "verified": 0, "created_at": ""
    }
    post_data = [dict(post) for post in posts]
    for post in post_data:
        try:
            post["views"] = int(post.get("views") or 0)
        except (TypeError, ValueError):
            post["views"] = 0
    profile_data["verified"] = bool(
        profile_data.get("verified")
        or any(post["views"] >= AUTO_VERIFY_VIEWS for post in post_data)
    )
    return render_template(
        "profile.html",
        profile=profile_data,
        posts=post_data,
        auto_verify_views=AUTO_VERIFY_VIEWS,
    )


@app.route("/pyv/share/<share_slug>/comments", methods=["POST"])
@limiter.limit("5 per minute; 30 per hour")
def api_add_comment(share_slug):
    if not re.fullmatch(r"[0-9a-f]{8}", share_slug):
    ...
    if len(body) > MAX_COMMENT_LENGTH:
        return jsonify({"error": f"Comment must be {MAX_COMMENT_LENGTH:,} characters or fewer."}), 400
    _ensure_profile(username)
    db.execute(
        "INSERT INTO comments (session_id, username, body, created_at) VALUES (?, ?, ?, ?)",
        (session_row["session_id"], username, body, datetime.utcnow().isoformat()),
    )
    db.commit()
    return jsonify({"message": "Comment posted.", "username": username, "body": body}), 201
...
def admin():
    ...
    profiles = [
        ...
        for profile in db.execute(
            "SELECT username, verified, created_at FROM profiles ORDER BY username"
        ).fetchall()
    ]
...
def _do_save(
    ...
    safe_description = str(description or "").strip()[:MAX_DESCRIPTION_LENGTH]
    ...
    db.execute(
        ...
        "max_executions, label, libraries, share_slug, username, description, tags, views) "
        "VALUES (?, ?, 0, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)",
        (session_id, encrypted, datetime.utcnow().isoformat(),
         owner_token_hash, expires_at, me, safe_label, safe_libraries, share_slug,
         safe_username, safe_description, safe_tags),
    )
    _ensure_profile(safe_username)
    db.commit()
    return (
        session_id, owner_token, expires_at, share_slug, safe_libraries,
        safe_username, safe_description, safe_tags, None
    )
...
def api_save():
    ...
    (
        sid, owner_token, expires_at, share_slug, safe_libraries,
        safe_username, safe_description, safe_tags, err
    ) = _do_save(
...
def api_upload():
    ...
    (
        ...
        label, libraries, username, description, tags
    )
    if err:
        return jsonify({"error": err}), 400
...
def api_info(session_id):
    ...
    row = db.execute(
        ...
        "expires_at, max_executions, label, libraries, share_slug, username, "
    ...
    return jsonify({
        ...
        "profile_verified": _profile_verified(row["username"], row["views"]),
...
def api_stats():
    ...
    total_views = db.execute("SELECT SUM(COALESCE(views, 0)) AS v FROM sessions").fetchone()["v"] or 0
    total_comments = db.execute("SELECT COUNT(*) AS c FROM comments").fetchone()["c"]
    verified_profiles = db.execute(
        "SELECT COUNT(*) AS c FROM profiles WHERE verified = 1"
    ).fetchone()["c"]
    return jsonify({
...
@app.route("/pyv/my/<session_id>", methods=["PATCH"])
@limiter.limit("20 per minute")
def api_owner_metadata(session_id):
    if not _validate_sid(session_id):
        return jsonify({"error": "Invalid session ID."}), 400
    row, err = _verify_owner(session_id)
    if err:
    ...
    if "label" in data:
        ...
        params.append(str(data["label"] or "").strip()[:MAX_LABEL_LENGTH])
    if "description" in data:
        updates.append("description = ?")
        params.append(str(data["description"] or "").strip()[:MAX_DESCRIPTION_LENGTH])
    if "tags" in data:
        updates.append("tags = ?")
        params.append(_normalize_tags(data["tags"]))
    if "username" in data:
        ...
        _ensure_profile(username)
    if not updates:
        return jsonify({"error": "No public profile fields to update."}), 400
    params.append(session_id)
    db = get_db()
    db.execute(f"UPDATE sessions SET {', '.join(updates)} WHERE session_id = ?", params)
    db.commit()
    return jsonify({"session_id": session_id, "message": "Public profile metadata updated."})


@app.route("/pyv/my/<session_id>/comments/<int:comment_id>", methods=["DELETE"])
@limiter.limit("20 per minute")
def api_owner_delete_comment(session_id, comment_id):
    if not _validate_sid(session_id):
        return jsonify({"error": "Invalid session ID."}), 400
    row, err = _verify_owner(session_id)
    if err:
...
def api_delete(session_id):
    if not _validate_sid(session_id):
        return jsonify({"error": "Invalid session ID."}), 400
    db = get_db()
    result = db.execute("SELECT 1 FROM sessions WHERE session_id = ?", (session_id,))
    if result.fetchone():
        _delete_session_data(session_id)
        deleted = True
    else:
...
@app.route("/pyv/admin/profile/<username>", methods=["PATCH"])
@require_admin_api
def api_admin_profile(username):
    normalized = _normalize_username(username)
    if normalized != username.lower():
    ...
    db = get_db()
    _ensure_profile(normalized)
    db.execute(
        "UPDATE profiles SET verified = ? WHERE username = ?",
        (1 if bool(data["verified"]) else 0, normalized),
    )
    db.commit()
    return jsonify({
        "username": normalized,
